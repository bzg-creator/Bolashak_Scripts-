        ...
        if r["is_drugoe"]:
            ...
            if r["old_id"]:
                ...
                if candidates:
                    ...
                    # (если их несколько — у всех должен быть один и тот же реальный Value)
                    best = candidates[0]
                    resolved_value   = best["new_value"]   # правильное название страны
                    resolved_old_val = best["old_value"]   # старое название в БД
                    resolved         = True
                    drugoe_resolved += 1
                else:
                    drugoe_unresolved += 1
            ...
            r["resolved_value"]   = resolved_value    # итоговый Value вместо "Другое"
            r["resolved_old_val"] = resolved_old_val  # OldValue для поиска в БД
            r["resolved"]         = resolved
        else:
...
def fetch_records_for_table(cursor, table_key, mapping_rows):
    ...
    for r in pairs:
        ...
        # Итоговое значение — для "Другое" это resolved_value, иначе new_value
